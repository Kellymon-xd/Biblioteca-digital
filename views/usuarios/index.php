...
<div class="container py-4">
  ...
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      ...
      <p class="text-muted mb-0">Listado de usuarios administrativos registrados.</p>
    </div>
              ...
              <?php foreach ($usuarios as $usuario): ?>
                <tr>
                  ...
                  <td class="fw-semibold">
                    <?= Sanitizador::html(trim(($usuario['nombre'] ?? '') . ' ' . ($usuario['apellido'] ?? ''))) ?>
                  </td>

                  <td>
                    <?= Sanitizador::html($usuario['email'] ?? '') ?>
                  </td>

                  <td>
                    <span class="badge bg-light text-dark border">
                      <?= Sanitizador::html($usuario['username'] ?? '') ?>
                    </span>
                  </td>

                  <td>
                    <span class="badge bg-light text-dark border">
                      <?= Sanitizador::html($usuario['rol'] ?? '') ?>
                    </span>
                  </td>
                  ...
                  <td class="text-end">
                    <div class="d-flex justify-content-end align-items-center gap-2 flex-nowrap">
                      <a
                        href="<?= BASE_URL ?>/index.php?mod=usuarios&accion=form&id=<?= (int)$usuario['id_usuario'] ?>"
                      ...
                      <?php if (!empty($usuario['bloqueado'])): ?>
                        <form
                          method="POST"
                          action="<?= BASE_URL ?>/index.php?mod=usuarios&accion=desbloquear"
                          class="m-0"
                          onsubmit="return confirm('¿Seguro que deseas desbloquear este usuario?');">
                      ...
                      <?php if (!empty($usuario['activo'])): ?>
                        <form
                          method="POST"
                          action="<?= BASE_URL ?>/index.php?mod=usuarios&accion=eliminar"
                          class="m-0"
            ...
            <ul class="pagination justify-content-center mb-0">
              <?php for ($i = 1; $i <= $paginas; $i++): ?>
                <li class="page-item <?= $i === ($pagina ?? 1) ? 'active' : '' ?>">
                  <a class="page-link" href="<?= BASE_URL ?>/index.php?mod=usuarios&pag=<?= $i ?>&q=<?= urlencode($busqueda ?? '') ?>">
                    <?= $i ?>
                  </a>
                </li>
              <?php endfor; ?>
